...
macro_rules! make_rtti_for {
    ($t:ty) => {
        GCRTTI {
            visit_references: |data, trace| unsafe {
            ...
            heap_size: |data| unsafe { (*data.add(8).cast::<$t>()).heap_size() + 8 },
        }
    };
    (finalize $t:ty) => {
        GCRTTI {
...
impl<T: HeapObject + ?Sized> Gc<T> {
    pub fn from_raw(ptr: *const T) -> Self {
        Self {
            marker: PhantomData,
            ptr: unsafe {
                NonNull::new_unchecked(ptr.cast::<RawGc>().sub(1) as *mut RawGc)
            },
        }
    }
...
impl TracerPtr {
    pub fn trace(self, val: &mut *mut RawGc) {
        unsafe {
            (*core::mem::transmute::<[usize; 2], *mut dyn Tracer>(self.tracer)).trace(
...
impl ConservativeTracer {
    pub fn add(&self, start: *mut *mut u8, end: *mut *mut u8) {
        unsafe {
            (&mut *(self.roots as *mut alloc::vec::Vec<(usize, usize)>)).push((
                start as usize,
                end as usize,
            ));
        }
