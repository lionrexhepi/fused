...
pub extern "C" fn immix_dump_summary() {
    unsafe {
        ...
        #[cfg(unix)]
        printf(b"GC stats: total=%.1f\n\0".as_ptr().cast(), runtime as libc::c_double);
        #[cfg(unix)]
        printf(
        ...
        #[cfg(unix)]
        printf(b"GC stats: collection=%.1f\n\n\0".as_ptr().cast(), stats.pause() as libc::c_double);

        #[cfg(unix)]
        printf(b"GC stats: collections count=%i\n\0".as_ptr().cast(), stats.collections() as i32);
        #[cfg(unix)]
        printf(
            b"GC summary: %.1fms collection (%i), %.1fms mutator, %.1f total (%f%% mutator, %f%% GC)\n\0"
                .as_ptr()
                .cast(),
...
pub type CollectRootsCallback = extern "C" fn(
    data: *mut u8,
    tracer: TracerPtr,
    cons_tracer: ConservativeTracer
);
...
impl Immix {
    ...
    fn collect_internal(&mut self, evacuation: bool, emergency: bool) {
        unsafe {
            ...
            #[cfg(feature = "threaded")]
            {
                ...
                ptls.atomic_gc_state().store(GC_STATE_WAITING, Ordering::Release);
    ...
    unsafe fn collect_roots(
        ...
        while scan < end {
            ...
            if
                PreciseAllocation::is_precise(ptr.cast()) &&
...
#[no_mangle]
pub fn immix_noop_visit(_: *mut u8, _: TracerPtr) {}
...
pub extern "C" fn immix_init(
    ...
    unsafe {
        ...
        if
            INIT.compare_exchange_weak(false, true, Ordering::SeqCst, Ordering::Relaxed) ==
            Ok(false)
        {
